	...
	def mlp_weights(self):
		...
		self.biases['b0_g'] = tf.Variable(tf.zeros([self.n_hidden]))
 		for i in range(1,self.n_layer):
			self.weights['h{}_f'.format(i)] = tf.Variable(tf.random_normal([self.n_hidden, self.n_hidden]))
			self.weights['h{}_g'.format(i)] = tf.Variable(tf.random_normal([self.n_hidden, self.n_hidden]))
			self.biases['b{}_f'.format(i)] = tf.Variable(tf.zeros([self.n_hidden]))
			self.biases['b{}_g'.format(i)] = tf.Variable(tf.zeros([self.n_hidden]))
		self.weights['out_f'] = tf.Variable(tf.random_normal([self.n_hidden, self.n_out]))
		self.weights['out_g'] = tf.Variable(tf.random_normal([self.n_hidden, self.n_out]))
