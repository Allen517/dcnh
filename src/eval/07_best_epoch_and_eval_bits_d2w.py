        ...
        for method in methods:
            for j in bit_set:
                ...
                for k in np.arange(range_from, range_to, gap):
                    src_lbs, target_lbs = _read_labels('{}.{}.test'.format(base_label_path, 0.7))
                    filename='{}/{}.d2w.bit_{}.train_0.7.times_{}.log'.format(base_log_path, method, j, k)
                    ...
                    if test_loss:
                        ...
                        if method=='dcnh-sp':
                            eval_hashing_model(src_lbs, target_lbs, '{}/douban.line.epoch5.node_embeddings_order2'.format(base_res_path), '{}/weibo.line.epoch5.node_embeddings_order2'.format(base_res_path), '{}/d2w.alp_model.{}.bit_{}.train_0.7.times_{}.epoch_{}'.format(base_model_path, method, j, k, max_idx*skip_step), cand_size, out_file, calc_mlp_sp_res, _hamming_distance)
                        if method=='pale-mlp':
                            eval_pale_model(src_lbs, target_lbs, '{}/douban.line.epoch5.node_embeddings_order2'.format(base_res_path), '{}/weibo.line.epoch5.node_embeddings_order2'.format(base_res_path), '{}/d2w.alp_model.{}.bit_{}.train_0.7.times_{}.epoch_{}'.format(base_model_path, method, j, k, max_idx*skip_step), cand_size, out_file, calc_mlp_res, _geo_distance)

                    filename='{}/{}.d2w_anchor_reg.bit_{}.train_0.7.times_{}.log'.format(base_log_path, method, j, k)
                    test_loss = get_test_loss(filename)
                    if test_loss:
                        test_loss = map(float, test_loss[::skip_step])
                        max_idx = np.argmax(test_loss)
                        fout.write('{}: max test loss:{} @{}\n'.format(filename, max(test_loss), np.argmax(test_loss)))
                        if method=='dcnh-sp':
                            eval_hashing_model(src_lbs, target_lbs, '{}/douban.line.epoch5.node_embeddings_order2'.format(base_res_path), '{}/weibo.line_anchor_reg.train_{}.epoch{}.node_embeddings_order2'.format(base_res_path, 0.7, e), '{}/d2w.alp_model.{}.bit_{}.line_anchor_reg.train_0.7.times_{}.epoch_{}'.format(base_model_path, method, j, k, max_idx*skip_step), cand_size, out_file, calc_mlp_sp_res, _hamming_distance)
...
get_model_perf('log', 'res', 'anchor_res', 'data/douban2weibo/d2w.anchor_links.labels', 'log_analy.d2w', 'mrr.d2w', 1, 6, 1)
