        ...
        for method in methods:
            for j in [.2, .3, 0.4, .5, .6, .7]:
                ...
                for k in np.arange(range_from, range_to, gap):
                    ...
                    filename='{}/{}.w2d.train_{}.times_{}.log'.format(base_log_path, method, j, k)
                    ...
                    if test_loss:
                        ...
                        max_iter = max_idx*skip_step if max_idx*skip_step>0 else 300
                        if method=='dcnh-sp':
                            eval_hashing_model(src_lbs, target_lbs, '{}/weibo.line.epoch5.node_embeddings_order2'.format(base_res_path), '{}/douban.line.epoch5.node_embeddings_order2'.format(base_res_path), '{}/w2d.alp_model.{}.train_{}.times_{}.epoch_{}'.format(base_model_path, method, j, k, max_iter), cand_size, out_file, calc_mlp_sp_res, _hamming_distance)
                        if method=='pale-mlp':
                            eval_pale_model(src_lbs, target_lbs, '{}/weibo.line.epoch5.node_embeddings_order2'.format(base_res_path), '{}/douban.line.epoch5.node_embeddings_order2'.format(base_res_path), '{}/w2d.alp_model.{}.train_{}.times_{}.epoch_{}'.format(base_model_path, method, j, k, max_iter), cand_size, out_file, calc_mlp_res, _geo_distance)
                    filename='{}/{}.w2d_anchor_reg.train_{}.times_{}.log'.format(base_log_path, method, j, k)
                    test_loss = get_test_loss(filename)
                    if test_loss:
                        test_loss = map(float, test_loss[::skip_step])
                        max_idx = np.argmax(test_loss)
                        ...
                        max_iter = max_idx*skip_step if max_idx*skip_step>0 else 300
                        if method=='dcnh-sp':
                            eval_hashing_model(src_lbs, target_lbs, '{}/weibo.line_anchor_reg.train_{}.epoch{}.node_embeddings_order2'.format(base_res_path, j, e), '{}/douban.line.epoch5.node_embeddings_order2'.format(base_res_path), '{}/w2d.alp_model.{}.line_anchor_reg.train_{}.times_{}.epoch_{}'.format(base_model_path, method, j, k, max_iter), cand_size, out_file, calc_mlp_sp_res, _hamming_distance)
