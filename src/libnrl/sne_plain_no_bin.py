...
class _SNE(object):
    ...
    def __init__(self, graph, lr=.001, rho=[.001,.001], mode_size=1000, rep_size=128, batch_size=100
        ...
        self.logger = LogHandler('sne_plain_no_bin')
    ...
    def _update_graph_by_lasso(self, x_set):
        keys = x_set.keys()

        delta_x=dict()
        for key in keys:
            shape = x_set[key].shape
            delta_x[key] = np.where(x_set[key]>=0, np.ones(shape), -np.ones(shape))
    ...
    def update_graph(self, x_set, batch, identity_labels):
        ...
        tmp_delta_x_lb, tmp_delta_codebook = self._update_graph_by_identity(x_set, identity_labels)
        tmp_delta_x_lasso = self._update_graph_by_lasso(x_set)
        delta_codebook = self.rho[0]*tmp_delta_codebook
        for key in keys:
            delta_x[key] = self.rho[0]*tmp_delta_x_lb[key]
            delta_x[key] += self.rho[1]*tmp_delta_x_lasso[key]
        # update by orders
        for key in keys:
            tmp_delta_x_order, delta_c[key], tmp_delta_codebook, delta_b_e[key]\
                        = self._update_graph_by_orders(x_set[key], key, batch)
            if key in delta_x:
                delta_x[key] += tmp_delta_x_order
            else:
                delta_x[key] = tmp_delta_x_order
    ...
    def get_lasso_loss(self, x_set):
        keys = x_set.keys()

        lasso_val = .0
        for key in keys:
            lasso_val += np.sum(np.abs(x_set[key]))

        return lasso_val

    def train_one_epoch(self):
        sum_loss = 0.0
        sum_identity_loss = 0.0
        batches = self.batch_iter()
        batch_id = 0
        for batch in batches:
            x_set = self.embeddings
            # print list(self.identity_labels)
            delta_x, delta_c, delta_codebook, delta_b_e = self.update_graph(self.embeddings, batch, self.identity_labels)
            self.codebook -= self.lr*delta_codebook
            graph_loss = 0
            for key in self.g.keys():
                self.embeddings[key] -= self.lr*delta_x[key]
                self.content_embeddings[key] -= self.lr*delta_c[key]
                self.b_e[key] -= self.lr*delta_b_e[key]
                graph_loss += self.get_one_graph_loss(self.embeddings[key], key, batch)
                # print 'u,v vec in {}:'.format(key), np.dot(x_set[key], self.codebook)
                # print key,'delta_embeddings:',np.clip(self.lr*delta_x[key],-1,1)
            identity_loss = self.get_identity_loss(self.embeddings, self.identity_labels)
            lasso_loss = self.get_lasso_loss(self.embeddings)
            cur_loss = graph_loss+self.rho[0]*identity_loss+self.rho[1]*lasso_loss
            sum_loss += cur_loss
            sum_identity_loss += identity_loss
            batch_id += 1
        ...
        self.logger.info('Epoch:{}, sum of loss:{!s}, sum of identity loss:{}'
                            .format(self.cur_epoch, sum_loss/batch_id, sum_identity_loss/batch_id))
        self.cur_epoch += 1
