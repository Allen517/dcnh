...
class _LINE(object):
    ...
    def build_graph(self):
        self.pos_h = tf.placeholder(tf.int32, [None]) # positive cases from (h->t)
        ...
        self.pos_h_e = tf.nn.embedding_lookup(self.embeddings, self.pos_h)
        self.pos_t_e = tf.nn.embedding_lookup(self.embeddings, self.pos_t)
        self.pos_t_e_context = tf.nn.embedding_lookup(self.context_embeddings, self.pos_t)
        self.pos_h_v_e = tf.nn.embedding_lookup(self.embeddings, self.pos_h_v)
