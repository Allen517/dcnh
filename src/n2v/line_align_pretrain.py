...
class _LINE_ALIGN_PRETRAIN(object):
    ...
    def _set_anchor_nds(self, mat, embedfile, anchorfile, order):
        anchors = self._read_anchors(anchorfile)
        embeddings = self._read_embeddings(embedfile)
        self.anchor_idx = set()
        for src_nd, target_nd in anchors:
            if not target_nd in self.look_up or not src_nd in embeddings:
                continue
            if len(mat[self.look_up[target_nd]])!=len(embeddings[src_nd]):
                self.logger.error('The length of embeddings at anchor nodes are illegal')
                break
            self.anchor_idx.add(self.look_up[target_nd])
            # if order==1:
            #     mat[self.look_up[target_nd]] = embeddings[src_nd][0:len(mat[self.look_up[target_nd]])]
            # if order==2:
            #     mat[self.look_up[target_nd]] = embeddings[src_nd][len(mat[self.look_up[target_nd]]):]
            mat[self.look_up[target_nd]] = embeddings[src_nd]
        return mat
...
class LINE_ALIGN_PRETRAIN(object):
    ...
    def __init__(self, graph, lr=.001, rep_size=128, batch_size=1000, epoch=10, 
        ...
        self.vectors = {}
        # training
        self.model = _LINE_ALIGN_PRETRAIN(graph, lr=lr, rep_size=rep_size
