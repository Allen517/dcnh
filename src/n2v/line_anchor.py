...
class _LINE_ANCHORREG_ALIGN_PRETRAIN(object):
    ...
    def _init_params(self, node_size, rep_size, embedfile, anchorfile):
        ...
        self.embeddings['order1'] = self._set_anchor_nds(self.embeddings['order1'], embedfile, anchorfile, 1)
        self.embeddings['order2'] = self._set_anchor_nds(self.embeddings['order2'], embedfile, anchorfile, 2)
    ...
    def _set_anchor_nds(self, mat, embedfile, anchorfile, order):
        ...
        for src_nd, target_nd in self.anchors:
            if not target_nd in self.look_up or not src_nd in self.src_embeddings:
                continue
            if len(mat[self.look_up[target_nd]])!=len(self.src_embeddings[src_nd]):
                self.logger.error('The length of embeddings at anchor nodes are illegal')
                break
    ...
    def _pre_train(self):
        ...
        for batch in batches:
            ...
            if order==2 or order==3:
                delta_c, delta_eh_o2 = self._pretrain_update_graph_by_order2(batch)
    ...
    def _update_graph_by_anchor_reg(self):
        ...
        for src_nd, target_nd in self.anchors:
            ...
            delta_eh = self._calc_delta_vec('nd_order2', self.look_up[target_nd]
                            , delta_eh, (self._cos_sim(src_emb, target_emb)*target_emb/np.dot(target_emb, target_emb)
                                            -src_emb/np.linalg.norm(src_emb)/np.linalg.norm(target_emb))
    ...
    def get_cur_batch_loss(self, t, batch):
        ...
        if t%DISPLAY_EPOCH==0:
            ...
            if self.order==2 or self.order==3:
                ...
                loss_order_2 += self.get_graph_loss_by_order2(batch)+anchor_loss
            if self.order==1:
                self.logger.info('Finish processing batch {} and loss from order 1:{}'
                                .format(t, loss_order_1))
            elif self.order==2:
                self.logger.info('Finish processing batch {} and loss from order 2:{} and anchor loss:{}'
    ...
    def save_embeddings(self, outfile):
        ...
        if self.order==3:
            ...
            for node, vec in vectors['node_embeddings_order1'].items():
                fout.write("{} {} {}\n".format(node,' '.join([str(x) for x in vec])
                                , ' '.join([str(x) for x in vectors['node_embeddings_order2'][node]])))
            fout.close()
...
class LINE_ANCHORREG_ALIGN_PRETRAIN(object):

    def __init__(self, graph, lr=.001, gamma=.01, rep_size=128, batch_size=1000, epoch=10, 
                    negative_ratio=5, order=3, table_size=1e8, outfile=None, 
                    embedfile=None, anchorfile=None, log_file='log', auto_stop=True):
        if not outfile or not embedfile or not anchorfile:
            print 'please give outfile and embedfile and anchorfile'
            return
        SAVING_EPOCH=1
        ...
        self.vectors = {}
        # training
        self.model = _LINE_ANCHORREG_ALIGN_PRETRAIN(graph, lr=lr, gamma=gamma, rep_size=rep_size
                            , batch_size=batch_size, negative_ratio=negative_ratio
                            , order=self.order, table_size=table_size
                            , embedfile=embedfile, anchorfile=anchorfile
                            , log_file=log_file)
    ...
    def save_embeddings(self, outfile):
        ...
        for c in vectors.keys():
            if 'node_embeddings' in c or 'content_embeddings' in c:
                ...
                fout.close()
        if self.order==3:
            fout = open('{}.node_embeddings_all'.format(outfile), 'w') 
            node_num = len(vectors[c].keys())
            fout.write("{} {}\n".format(node_num, self.rep_size*2))
            for node, vec in vectors['node_embeddings_order1'].items():
                fout.write("{} {} {}\n".format(node,' '.join([str(x) for x in vec])
